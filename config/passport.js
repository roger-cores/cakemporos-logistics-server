...
module.exports = function(passport){
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        ID.findOne({ email :  email }, function(err, user) {
            ...
            if (!user)
                return done({err: "no user"}, false, req.flash('loginMessage', 'No user found.')); // req.flash is the way to set flashdata using connect-flash

            // if the user is found but the password is wrong
            if (!user.validPassword(password))
                return done({err: "error"}, false, req.flash('loginMessage', 'Oops! Wrong password.')); // create the loginMessage and save it to session as flashdata
